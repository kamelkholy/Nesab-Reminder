...
export async function sendZakatReminder(
  ...
  try {
    ...
    const eligibleList = summary.eligibleAssets
      .filter(a => a.hawlComplete && a.zakatAmount > 0)
      .map(
        a =>
          `<tr>
            ...
            <td style="padding:8px;border:1px solid #ddd">${a.hijriAcquisitionDate}</td>
            <td style="padding:8px;border:1px solid #ddd">${a.zakatAmount.toFixed(2)} EGP</td>
          </tr>`
      )
      .join('');
    ...
    const html = `
      <div style="font-family:Arial,sans-serif;max-width:600px;margin:0 auto">
        ...
        <table style="border-collapse:collapse;width:100%">
          <thead>
            <tr style="background:#1a7a4c;color:white">
              ...
              <th style="padding:8px;border:1px solid #ddd">Hijri Date</th>
              <th style="padding:8px;border:1px solid #ddd">Zakat</th>
            </tr>
          </thead>
          <tbody>
            ${eligibleList}
        ...
        <p style="margin-top:16px;color:#666;font-size:0.9em">
          Zakat is calculated at 2.5% of eligible wealth that has been held for one full Hijri year (hawl)
          and exceeds the Nisab threshold (85 grams of gold equivalent).
        </p>
